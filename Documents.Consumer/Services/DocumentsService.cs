...
namespace Documents.Consumer.Services
{
    public class DocumentsService : IDocumentsService
    {
        ...
        public async Task<DocumentBusMessage?> GetAsync(string id) =>
            await _documentsCollection.Find(x => x.DocumentId == Guid.Parse(id)).FirstOrDefaultAsync();
        ...
        public async Task UpdateAsync(string id, DocumentBusMessage updatedDocumentBusMessage) =>
            await _documentsCollection.ReplaceOneAsync(x => x.DocumentId == Guid.Parse(id), updatedDocumentBusMessage);

        public async Task RemoveAsync(string id) =>
            await _documentsCollection.DeleteOneAsync(x => x.DocumentId == Guid.Parse(id));
    }
}
